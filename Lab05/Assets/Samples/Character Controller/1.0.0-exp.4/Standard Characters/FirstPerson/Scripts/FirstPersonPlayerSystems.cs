...
{
    private InputActionAssetSharedComponentData m_InputActionAsset;
    ...
    {
        InputActionAssetSharedComponentData tmp = new InputActionAssetSharedComponentData();
        Entities
            .WithoutBurst()
            .ForEach((in InputActionAssetSharedComponentData data) => {
                tmp.Value = data.Value;
            }).Run();
        if (tmp.Value != null)
        {
            m_InputActionAsset = tmp;
            // have to initialize here
            m_InputActionAsset.Initialize();
            Debug.Log("Sucessfully Set New Input System Controls");
        }
        else Debug.LogError("Failed to find a component iwth an InputActionAsset!");
    }
    ...
    protected override void OnUpdate()
    {
        ...
        {
            playerInputs.ValueRW.LookInput = new float2();
            playerInputs.ValueRW = m_InputActionAsset.GenerateFirstPersonPlayerPrefs(fixedTick);
